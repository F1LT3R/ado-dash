...
function formatReviewers(reviewers) {
	...
	if (counts.rejected) parts.push(`❌${counts.rejected}`)
	if (counts.waiting) parts.push(`⏳${counts.waiting}`)
	if (counts.suggestions) parts.push(`🔄${counts.suggestions}`)
	return parts.join(' ')
}
...
export function formatPRPanel(prs, rows, cols, changedIds = new Set(), currentUser = '', cursorIndex = -1) {
	...
	for (let i = 0; i < availableRows && i < sorted.length; i++) {
		...
		const statusEmoji = isDraft ? '📝' : (STATUS_EMOJI[status] || '⏳')
